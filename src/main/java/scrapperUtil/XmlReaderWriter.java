...
public class XmlReaderWriter {
	...
	private static final String xmlDir = "/Realtor Agents Scraper/";
	...
	public <T> T fromXml(T classType , String xmlSettingsName) {
		...
		if (new File(userHome+xmlDir+xmlSettingsName).exists()) {
			...
			try {
				...
				JAXBContext context = new JAXBContextFactory().createContext(new Class[] { classType.getClass() }, null);

				fromXml = (T) context.createUnmarshaller().unmarshal(new FileReader(userHome+xmlDir+xmlSettingsName));
